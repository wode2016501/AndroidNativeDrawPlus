...
const char *getClipText() {
	...
	static jmethodID getClipTexto = nullptr;
	if (!getClipTexto) {
		getClipTexto = env->GetStaticMethodID(entryClass, "getClipText",
				"()Ljava/lang/String;");
	}
	auto jstr = (jstring) env->CallStaticObjectMethod(entryClass, getClipTexto);
...
#endif
//	ImGui::SetNextWindowSize({2000, 100}, ImGuiCond_Once);
	...
	int gpuz=open("/sys/kernel/gpu/gpu_busy",0);
	if(gpuz<0) {
		printf("打开/sys/kernel/gpu/gpu_busy错误\n");
		return ;
	}
	int count=500; 
	char mappedData[50];
	...
	while (flag) {
		ImGui::NewFrame();
		if(a){
			ImGui::SetNextWindowSize({680, 100}, ImGuiCond_Once);
			a=0;
		}
		ImGui::Begin("My Window", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize);
		ImVec4 redColor = { 0.5f, 0.5f, 0.5f, 0.5f };  
		ImGui::PushStyleColor(ImGuiCol_WindowBg, redColor);
		read(fd,mappedData,9); 
		lseek(fd, 0, 0);
		lseek(gpuz, 0, 0);
		samples[count-1]=atof(p);
		for (int i = 0; i < count-1; i++)
			samples[i]=samples[i+1];
		read(gpuz,mappedData,9);
		ret=atoi(mappedData); 
		sprintf(mappedData,"fps: %.1f, gpu: %d%%",samples[count-1],ret); 
